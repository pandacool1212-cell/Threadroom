...
- **Canvas/garment sync** (what everyone is drawing, which garment is
  ...
  once configured. It's short-poll based (every ~1.5s, plus a heartbeat)
  rather than a live socket, so updates land within a couple of seconds —
  fine for co-painting, not meant for millisecond-precision cursor tracking.
