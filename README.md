...
out. For anything longer-lived, in **Realtime Database → Rules**, paste this.
It covers every path the app uses:

- `tr_storage` — room canvas snapshots, photo assets, fallback draw events
- `tr_photos` — high-quality photo bytes (multi-user image stamps)
- `tr_draw` — live stroke / stamp / photo events (near-instant co-op drawing)
  ...
  "rules": {
    ...
    "tr_photos": {
      "$room": {
        ...
        "$assetId": {
          ".write": true,
          ".validate": "newData.hasChildren(['src','ts']) && newData.child('src').isString() && newData.child('src').val().length < 1500000"
        }
      }
    },
    "tr_draw": {
      "$room": {
        ...
        ".write": true,
        "$eventId": {
          ".validate": "newData.hasChildren(['from','ts']) && newData.child('ts').isNumber()"
        }
      }
    }
  }
...
**Important:** After pasting rules, click **Publish**. Until rules allow
`tr_photos` and `tr_draw`, photos and live strokes may only show on the
device that painted them.
...
- The biggest remaining source of lag on a slow connection is the size of
  what's sent, not how often: each push is a JPEG snapshot of the paint
  canvas at `NET_TEX` (2560px, quality ~0.94 in `pushRoomState()`), plus a
  separate high-quality photo asset when someone stamps an image. Photos are
  stored once under `photoasset:{room}:{id}` / `tr_photos/{room}/{id}` and
  referenced by id so they stay sharp on every peer without re-uploading the
  full file on every stroke.
